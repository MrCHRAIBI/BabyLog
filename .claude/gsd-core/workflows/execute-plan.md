...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `D:/projets/react/BabyLog/.claude/gsd-core/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `D:/projets/react/BabyLog/.claude/gsd-core/templates/summary.md`.
