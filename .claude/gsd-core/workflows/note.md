...
- **Global scope**: `D:/projets/react/BabyLog/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/projets/react/BabyLog/.claude/notes/`)
...
2. Glob `D:/projets/react/BabyLog/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/projets/react/BabyLog/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/projets/react/BabyLog/.claude/notes/` — works in any directory
