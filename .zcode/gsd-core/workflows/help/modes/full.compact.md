...
**`/gsd:spike --wrap-up`** — Curates spikes one-at-a-time (include/exclude/partial/UAT), generates a project skill under `./.zcode/skills/spike-findings-[project]/`, writes `.planning/spikes/WRAP-UP-SUMMARY.md`, adds a CLAUDE.md auto-load line.

Usage: `/gsd:spike --wrap-up`

**`/gsd:sketch --wrap-up`** — Same curation flow for sketches, generating `./.zcode/skills/sketch-findings-[project]/` with design decisions/CSS/HTML structures.
...
**`/gsd:capture --note <text>`** — Zero-friction timestamped note to `.planning/notes/` (or `D:/projets/react/BabyLog/.zcode/notes/` globally). Subcommands: append (default), list, promote (note → todo). Works without a project.
