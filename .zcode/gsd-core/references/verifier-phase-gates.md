...
- **Per-stack verification patterns:** before Step 4 (artifact verification) on an unfamiliar stack, Read `D:/projets/react/BabyLog/.zcode/gsd-core/references/verification-patterns.md` — the grep catalog for React/Next.js components, API routes, database schema, and the universal stub patterns. Read it lazily (only the sections for the stack under verification); it is too large to load wholesale on every run.
- **Canonical report shape:** the emitted VERIFICATION.md follows `@D:/projets/react/BabyLog/.zcode/gsd-core/templates/verification-report.md` — the template whose Guidelines and row shapes `src/uat.cts` treats as canonical when consuming verification output.
